...
def shift_delivery_in_route(route, delivery_id, action: bool, size: int, couriers=None, deliveries=None, distance_matrix=None):
    ...
    # print("Is route feasible: ", is_feasible(route, couriers, deliveries, distance_matrix))

def change_delivery_route(route, delivery_id, new_route, couriers=None, deliveries=None, distance_matrix=None):
    ...
    # print("Is route feasible: ", is_feasible(new_route, couriers, deliveries, distance_matrix))



def swap_delivery(routes, size, couriers=None, deliveries=None, distance_matrix=None):
    # Choose two random routes
    big_enough_routes = [route for route in routes if len(route.stops) >= 2]
    random_route_remove_idx = random.randint(0, len(big_enough_routes) - 1)
    
    # Ensure the chosen route has at least one stop
    while len(big_enough_routes[random_route_remove_idx].stops) == 0:
        random_route_remove_idx = random.randint(0, len(big_enough_routes) - 1)
    random_route_remove_id = big_enough_routes[random_route_remove_idx].rider_id
    

    small_routes = [route for route in routes if len(route.stops) <= 6]
    random_route_add_idx = random.randint(0, len(small_routes) - 1)
    random_route_add_id = small_routes[random_route_add_idx].rider_id
    ...
    if random_route_remove_id == random_route_add_id:  # Same route
        random_idx = random.randint(0, len(big_enough_routes[random_route_remove_idx].stops) - 1) 
        delivery_id = big_enough_routes[random_route_remove_idx].stops[random_idx]  # Randomly choose a delivery to shift
        ...
        shift_delivery_in_route(big_enough_routes[random_route_remove_idx], delivery_id, action, size, couriers, deliveries, distance_matrix)

    else:  # Different routes: remove from one route and add to another
        random_idx = random.randint(0, len(big_enough_routes[random_route_remove_idx].stops) - 1)
        delivery_id = big_enough_routes[random_route_remove_idx].stops[random_idx]  # Randomly choose a delivery to remove

        # Check if the delivery already exists in the target route to avoid duplicating it
        # if delivery_id in small_routes[random_route_add_idx].stops:
        #     print(f"Delivery {delivery_id} already exists in the target route, no swap made.")

        if delivery_id not in small_routes[random_route_add_idx].stops:
            # Move the delivery to the new route
            change_delivery_route(big_enough_routes[random_route_remove_idx], delivery_id, small_routes[random_route_add_idx], couriers, deliveries, distance_matrix)
...
def simulated_annealing(initial_solution, initial_temp, cooling_rate, max_iterations, size_neighborhood=1, couriers=None, deliveries=None, distance_matrix=None):
    ...
    for iter in range(max_iterations):
        ...
        new_solution = swap_delivery(new_solution, size_neighborhood, couriers, deliveries, distance_matrix) # Modify the new solution
